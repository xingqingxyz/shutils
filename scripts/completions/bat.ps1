...
@(if ($commandAst.CommandElements[1].Extent.Text -eq 'cache') {
  ...
  elseif ($prev.Extent.Text -eq '--theme') {
    bat --list-themes | ForEach-Object {
      [CompletionResult]::new($_, $_, [CompletionResultType]::ParameterValue, $_)
    }
  }
  elseif (@('-l', '--language').Contains($prev.Extent.Text)) {
    @('bash', 'zsh', 'fish', 'elvish', 'pwsh', 'ps1', 'sh', 'py', 'python', 'js', 'ts', 'rs', 'go', 'man', 'help', 'awk', 'md', 'ini', 'json', 'jsonc', 'yml', 'xml', 'html', 'cs', 'vb', 'cpp', 'c', 'lua', 'codeql', 'sql', 'rb', 'makefile', 'cmake', 'gql', 'tsx', 'mdx', 'svelte', 'vue', 'angular', 'astro', 'css', 'scss', 'sass', 'stylus', 'htmx', 'rst', 'ipynb') | ForEach-Object {
      [CompletionResult]::new($_, $_, [CompletionResultType]::ParameterValue, $_)
    }
  }
  else {
  ...
  }) | Where-Object { $_.ListItemText.StartsWith($wordToComplete) }
