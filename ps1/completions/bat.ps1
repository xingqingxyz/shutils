...
Register-ArgumentCompleter -Native -CommandName bat -ScriptBlock {
  ...
  $items = switch ($command) {
    '' {
      switch ($prev) {
        ...
        '--style' {
          'default', 'full', 'auto', 'changes', 'header', 'header-filename', 'header-filesize', 'grid', 'rule', 'ship', 'plain', 'numbers'
          break
        }
        '--theme' {
  ...
  if ($items[0] -isnot [CompletionResult]) {
    $items = $items.ForEach{ [CompletionResult]::new($_) }
  }
  $items | Where-Object CompletionText -Like $wordToComplete*
}
