  ...
  @(switch ($prev) {
      ...
      '--tmux' {
        ...
        if ($options.Length -eq 1) {
          @('left', 'right', 'down', 'up') | ForEach-Object { "'$_'" }
        }
        else {
          @(20, 25, 30, 40, 50, 60, 70, 75, 80, 90, 100) | ForEach-Object { "'$(@(($options | Select-Object -SkipLast 1); "$_%") -join ',')'" }
        }
        break 
      }
      ...
      '--preview-window' { 
        ...
        @(
          ...
          @(20, 25, 30, 40, 50, 60, 70, 75, 80, 90, 100) | ForEach-Object { "$_%" }
        ) | ForEach-Object { "'$(@(($options | Select-Object -SkipLast 1); $_) -join ',')'" }
        break 
      }
      '--with-shell' { @('bash -c', 'pwsh -nop -c', 'zsh -c', 'fish -c', 'cmd /d /c', 'powershell -nop -c') | ForEach-Object { "'$_'" }; break }
      '--walker' {
        $options = $wordToComplete.TrimStart("'").Split(',')
        @('file', 'dir', 'follow', 'hidden') | ForEach-Object { "'$(@(($options | Select-Object -SkipLast 1); $_) -join ',')'" }
        break
      }
      Default {
