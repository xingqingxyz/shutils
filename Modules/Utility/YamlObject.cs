...
{
    #region Helpers
    public readonly record struct ConvertToYamlContext(
        int MaxDepth,
        bool EnumsAsStrings,
        PSCmdlet Cmdlet,
    ...
    public class StringEnumConverter : YamlConverter
    {
        ...
        public override object? Read(YamlReader reader, Type typeToConvert)
        {
            ...
            try
            {
                ...
                else if (reader.TokenType == YamlTokenType.Integer)
                {
                    isInt = true;
    ...
    public static object? ConvertFromYaml(string input, out ErrorRecord? error)
    {
        return ConvertFromYaml(input, returnHashtable: false, out error);
    }
    ...
    {
        return ConvertFromYaml(input, returnHashtable, maxDepth: 1024, out error);
    }
    ...
    {
        return ConvertFromYaml(input, returnHashtable, maxDepth, false, out var store, out error);
    }

    /// <summary>
    /// Convert a YAML string back to an object of type <see cref="PSObject"/> or
    /// <see cref="Hashtable"/> depending on parameter <paramref name="returnHashtable"/>.
    /// </summary>
    /// <param name="input">The YAML text to convert.</param>
    /// <param name="returnHashtable">True if the result should be returned as a <see cref="Hashtable"/>
    /// instead of a <see cref="PSObject"/>.</param>
    ...
    /// <returns>A <see cref="PSObject"/> or a <see cref="Hashtable"/>
    /// if the <paramref name="returnHashtable"/> parameter is true.</returns>
    internal static object? ConvertFromYaml(
        string input,
        bool returnHashtable,
        int maxDepth,
        bool returnMetadataStore,
        out YamlMetadataStore? store,
        out ErrorRecord? error
    )
    {
        ...
        try
        {
            ...
            var deserializer = builder.Build();
             var obj =        deserializer.Deserialize<YamlDocument>(input);
            return returnHashtable
                ? PopulateHashTableFromObject(obj, out error)
                : PopulateFromObject(
                    obj,
        ...
        catch (YamlException ye)
        {
            ...
            throw new ArgumentException(msg, ye);
        }
    }

    private static object? PopulateFromObject(
        YamlTable obj,
    ...
    {
        error = null;
        var result = new PSObject(obj.Count);
        ...
        foreach (var entry in obj)
        {
            ...
            switch (entry.Value)
            {
                case YamlTableArray list:
                {
                    // Array
                    var listResult = PopulateFromYamlTableArray(list, out error);
                    if (error != null)
                    {
                        return null;
                    }
                ...
                {
                    // Array
                    var listResult = PopulateFromYamlArray(list, out error);
                    if (error != null)
                    {
                        return null;
                    }
                ...
                {
                    // Dictionary
                    var dicResult = PopulateFromObject(
                        dic,
    ...
    private static object? PopulateFromYamlTableArray(YamlTableArray list, out ErrorRecord? error)
    {
        error = null;
    ...
    // This function is a clone of PopulateFromList using YamlArray as input.
    private static object? PopulateFromYamlArray(YamlArray list, out ErrorRecord? error)
    {
        error = null;
        var result = new object?[list.Count];
        var i = 0;
        ...
        foreach (var element in list)
        {
            switch (element)
            {
                case YamlArray subList:
                    // Array
                    result[i++] = PopulateFromYamlArray(subList, out error);
                    if (error != null)
                    {
                        return null;
                    }
    ...
    // This function is a clone of PopulateFromDictionary using YamlTable as an input.
    private static object? PopulateHashTableFromObject(YamlTable obj, out ErrorRecord? error)
    {
        error = null;
        OrderedHashtable result = new(obj.Count);
        foreach (var entry in obj)
        {
            switch (entry.Value)
            {
                case YamlTableArray list:
                {
                    var listResult = PopulateHashTableFromYamlTableArray(list, out error);
                    if (error != null)
                    {
                        return null;
                    }
                ...
                {
                    // Array
                    var listResult = PopulateHashTableFromYamlArray(list, out error);
                    if (error != null)
                    {
                        return null;
                    }
                ...
                {
                    // Dictionary
                    var dicResult = PopulateHashTableFromObject(dic, out error);
                    if (error != null)
                    {
                        return null;
                    }
    ...
    private static object? PopulateHashTableFromYamlTableArray(
        YamlTableArray list,
        out ErrorRecord? error
    )
    {
        ...
        {
            // Dictionary
            result[i++] = PopulateHashTableFromObject(element, out error);
            if (error != null)
            {
                return null;
            }
    ...
    // This function is a clone of PopulateFromList using YamlArray as input.
    private static object?[]? PopulateHashTableFromYamlArray(YamlArray list, out ErrorRecord? error)
    {
        error = null;
        var result = new object?[list.Count];
        var i = 0;
        ...
        foreach (var element in list)
        {
            switch (element)
            {
                case YamlArray subList:
                    // Array
                    result[i++] = PopulateHashTableFromYamlArray(subList, out error);
                    if (error != null)
                    {
                        return null;
                    }
                ...
                case YamlTable dic:
                    // Dictionary
                    result[i++] = PopulateHashTableFromObject(dic, out error);
                    if (error != null)
                    {
                        return null;
                    }
    ...
    #endregion ConvertFromYaml

    #region ConvertToYaml

    /// <summary>
    /// Convert an object to YAML string.
    /// </summary>
    public static string? ConvertToYaml(object objectToProcess, in ConvertToYamlContext context)
    {
        try
        {
    ...
    /// <summary>
    /// Return an alternate representation of the specified object that serializes the same YAML, except
    ...
    /// <returns>An object suitable for serializing to YAML.</returns>
    private static object? ProcessValue(
        object? obj,
        int currentDepth,
        in ConvertToYamlContext context
    )
    {
        ...
        else
        {
            ...
            {
                // Enums based on System.UInt64 are not YAML-serializable
    ...
    /// <summary>
    /// Return an alternate representation of the specified dictionary that serializes the same YAML, except
    ...
    /// <summary>
    /// Return an alternate representation of the specified collection that serializes the same YAML, except
    /// that any contained properties that cannot be evaluated are treated as having the value null.
    /// </summary>
    ...
    /// <summary>
    /// Return an alternate representation of the specified aggregate object that serializes the same YAML, except
    /// that any contained properties that cannot be evaluated are treated as having the value null.
    ///
